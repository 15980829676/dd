Component({
  ...
  methods: {
    ...
    _updateScrollTopChange(scrollTop) {
      const stickies = this.getRelationNodes('../index-item/index');
      if (stickies.length > 0) {
        stickies.forEach((item) => {
          if (item) {
            item.updateScrollTopChange({
              scrollTop, 
              top: item.data.top, 
    ...
    handlerScroll(event) {
      ...
      this._updateScrollTopChange(scrollTop)
    ...
    getCurrentItem(index) {
      if (index <= 0) index = 0 // 修改2019/1/11
      const indexItems = this.getRelationNodes('../index-item/index');
      let result = {};
    ...
    setTouchStartVal() {
      ...
      query.select(className).boundingClientRect((res) => {
        // console.log(className,res) 修改 2019/1/11
        console.log(res)
        if (res) {
          this.setData({
            startTop: res.top
