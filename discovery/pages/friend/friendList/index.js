...
Page({
  ...
  onLoad: function (options) {
    ...
    this.navItem(this.data.typeNum)
  ...
  imgloaderr(ev) {
    ...
    friend_data[key].ImagetextStart = '图片加载失败'
    this.setData({
      friend_data
    })
 ...
 navItem(typeNum) {
    typeNum = typeNum || {}
    let tagId = typeNum.tagId || ''
    let type = typeNum.type || '' 
    let that = this  
   if (type == 0) {
     ...
     that.load_data(tagId)
   } else if (type == 1) {
     
      wx.setNavigationBarTitle({
        title: '短视频'
      })
     that.load_dataVideo(tagId)
   } else if (type == 2) {
      wx.setNavigationBarTitle({
        title: '文章'
      })
     that.load_dataText(tagId)

    }
  },
  //加载数据
  load_data: function (seaValue) {
    let that = this
    let seaValueText = seaValue || '';
    wx.request({
      url: that.data.httpsApi + 'test/querySelExtension', //9 所有类型 1/2/3 视频/图片/文章  第一个
  ...
  //视频加载
  load_dataVideo: function (seaValue) {
    let that = this
    let seaValueText = seaValue || '';
    wx.request({
      url: that.data.httpsApi + 'test/querySelExtension', //9 所有类型 1/2/3 视频/图片/文章  第一个
